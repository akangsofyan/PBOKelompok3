...
from Database_kantin import *
import datetime
import time
import sys

db = Database_akun('store_kantin.db')
daf_pen = []
meja = 24
order = 0;ln='------------------------------------------'
...
class Autentikasi(QWidget):
    def __init__(self):
        ...
        label_job = QLabel('<font size="4">Pilih Job : </font>')
        layout.addWidget(label_job, 5,0)
        layout.setRowMinimumHeight(2, 30)

        # lst = QListWidget() #un-comment to see listbox table users
        # for row in db.fetch():
        #     lst.addItem(' '.join(list(map(str,(row)))))
        # layout.addWidget(lst,6,0,1,2)

        self.cbx_job = QComboBox()
        self.cbx_job.addItems(['Kasir', 'Pelayan', 'Penjual'])
        layout.addWidget(self.cbx_job,5,1)
        # print(cbx_job.currentText())
    ...
    def check_password(self):
        ...
        if self.lineEdit_username.text() == '' or self.lineEdit_password.text() == '':
            ...
            if self.lineEdit_username.text() in [list(map(str,row))[1] for row in db.fetch()] :
                for row in db.fetch():
                    if self.lineEdit_username.text() == list(map(str,(row)))[1]:
                        if self.lineEdit_password.text() == list(map(str,(row)))[2]:
                            msg.setText('Sukses')
    ...
    def signin(self):
        msg = QMessageBox()
        db.insert(self.lineEdit_username.text(),self.lineEdit_password.text(),self.cbx_job.currentText())
